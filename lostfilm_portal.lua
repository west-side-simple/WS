	...
	if not retAdr:match('&lostfilm') and not inAdr:match('lostfilm%.tv%/new') and not inAdr:match('lostfilm%.tv%/series%/%?type') then
		...
		local name, c, s, e
			for ww in answer:gmatch('markEpisodeAsWatched.-</tr>') do
				...
				newadr = newadr:gsub('%/%/', '/'):gsub('https:%/', 'https://')
				...
				serial_str = '<a href = "simpleTVLua:m_simpleTV.Control.PlayAddress(\'' .. inAdr:gsub('season%_.-$', '') .. '\')"><center><img src="https://static.lostfilm.tv/Images/' .. c .. '/Posters/poster.jpg" height="' .. 80*masshtab .. '">'
				a1[j] = {}
				a1[j].Id = j
...
------------------
	...
	m_simpleTV.Control.ExecuteAction(108,1)
elseif inAdr:match('lostfilm%.tv%/series%/%?type') then
		local title = 'lostfilm'
		local url1 = 'https://www.lostfilm.tv'
		m_simpleTV.Interface.SetBackground({BackColor = 0, BackColorEnd = 255, PictFileName = 'https://www.tarablog.net.ua/wp-content/uploads/2014/01/lostfilm1.jpg', TypeBackColor = 0, UseLogo = 3, Once = 1})
		local rc, answer = m_simpleTV.Http.Request(session, {url = retAdr})
		...
		local retAdr = background_chanel
		local t, i, k, serials_str = {}, 1, 1, '<table width="99%"><tr>'
		local name, adr, logo
		answer:gsub('\n', ' ')
--		for w in answer:gmatch('<div class="row">(.-)markEpisodeAsWatched') do
		for w in answer:gmatch('<div class="row">(.-<div class="mark%-green%-box".-</div>)') do
				...
				t[i].name = name:gsub(',', ' '):gsub('%s%s+', ' ')
				t[i].address = url1 .. adr
				t[i].video_desc = '<td style="padding: 10px 10px 5px; color: #EBEBEB; vertical-align: middle;"><a href = "simpleTVLua:m_simpleTV.Control.PlayAddress(\'' .. t[i].address .. 
